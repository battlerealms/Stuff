...
public void OnPluginStart()
{
	...
	g_hCVDisableHardDrop = CreateConVar("sm_tetris_disableharddrop", "0", "Disable hard dropping the tetrimino instantly to the floor with the spacebar?", CVAR_FLAGS, true, 0.0, true, 1.0);
	g_hCVDisableSounds = CreateConVar("sm_tetris_disablesounds", "0", "Disable all tetris sounds?", CVAR_FLAGS, true, 0.0, true, 1.0);
	g_hCVOnlyDead = CreateConVar("sm_tetris_onlydead", "0", "Player has to be dead to play tetris?", CVAR_FLAGS, true, 0.0, true, 1.0);
	g_hCVDisableStats = CreateConVar("sm_tetris_disableinfopanel", "0", "Disable showing stats like score/cleared bars in a (Key)Hint panel while playing?", CVAR_FLAGS, true, 0.0, true, 1.0);
	...
	g_bIsL4D = GetEngineVersion() == Engine_Left4Dead;
...
public void ConVar_OnChange(ConVar convar, const char[] oldValue, const char[] newValue)
{
...
public Action Cmd_Tetris(int client, int args)
{
	if(!client)
	{
...
public int Menu_MainMenu(Menu menu, MenuAction action, int param1, int param2)
{
	if(action == MenuAction_Select)
	{
		...
		if(StrEqual(info, "startnew"))
		{
			...
			g_hPlayerGameThink[param1] = CreateTimer(view_as<float>(g_fDifficulty[param1]), Timer_PlayerGameThink, GetClientUserId(param1), TIMER_REPEAT|TIMER_FLAG_NO_MAPCHANGE);
...
public int Menu_HandleHighscore(Menu menu, MenuAction action, int param1, int param2)
{
	if(action == MenuAction_Select)
	{
		DisplayTetrisMainMenu(param1);
...
public int Menu_HandleTop5(Menu menu, MenuAction action, int param1, int param2)
{
	if(action == MenuAction_Select)
	{
		char info[32];
...
public int Panel_HandleGame(Menu menu, MenuAction action, int param1, int param2)
{
	if(action == MenuAction_Select)
	{
...
public Action Timer_DrawInfoPanel(Handle timer, any userid)
{
	...
	if (!g_bIsCSGO || !g_bIsL4D || !g_bIsL4D2)
	{
		int iMatrix[4][4];
